<template>
  <section class="table-page-section MTeQuery">
    <div class="table-page-wrapper">
      ...
      <div class="search-block" v-if="table.inputControlList.length > 0 || table.addBtn">
        ...
        >
          <MTDraggableSearch
            ...
            :uiModel="uiModel"
          ></MTDraggableSearch>
          <MTSearchItem v-else :controlList="table.inputControlList" :uiModel="uiModel"></MTSearchItem>
      ...
      <div class="btns-block">
        <div class="btns-block-left">
          <template v-if="table.addBtn">
            ...
            >{{ $t(`${item.funcName}Btn`) }}</el-button>
          </template>
        </div>
        <div class="btns-block-right">
          <el-tooltip class="item" effect="dark" content="切换展示" placement="bottom-start">
            <el-button
              class="change-tableTypeBtn"
              style="margin-right: 10px;"
              type="primary"
              size="small"
              circle
              plain
              @click="handleClick_listType"
            >
              <svg-icon
                :iconClass="
                  listType == 'table' ? 'icon-list-card' : 'icon-list-table'
                "
          ...
          <el-dropdown :hide-on-click="false" @command="handleCommand">
            <el-button icon="el-icon-more " type="primary" size="small" circle plain></el-button>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(it, index) in table.tableHeader" :key="index" :command="it">
      ...
      <div class="content-block">
        ...
        <div class="right-content" :style="{ width: `calc(100% - ${table.leftWidth})` }">
          ...
          ></MTeQueryCard>
          <!--表格渲染-->
          <div class="table-block" v-if="listType == 'table'">
            <el-table
              ref="multipleTable"
              :node-key="table.rowKey"
              ...
              >
                <template slot-scope="scope">
                  ...
                  <template v-if="dropCol[index].type == 'photo'">
                    <ImgView class="table-img" :imgSrc="scope.row[dropCol[index].prop]"></ImgView>
                  </template>
                  <!-- <template
                    v-else-if="
                  ...
                  >
                    <span
                      ...
                      v-if="scope.row[dropCol[index].prop] == 1"
                      >{{ filterName(dropCol[index], scope.row) }}</span
                    >
                  ...
                  <template
                    ...
                    "
                  >{{ filterName(dropCol[index], scope.row) }}</template>
                  <template v-else-if="dropCol[index].tableSlot">
                    <slot :name="dropCol[index].prop" :data="scope">slot的默认内容</slot>
                  </template>
                  ...
                  <template v-else>{{ scope.row[dropCol[index].prop] }}</template>
                </template>
              </el-table-column>
...
@Component({
  components: {
    ...
    Sortable,
    MTeQueryCard: (resolve) =>
      require(["@/controls/MTeQueryCard.vue"], resolve),
  },
})
export default class MTeQuery extends Vue {
  ...
  private listType: string = "table"; //列表展示类型  table/card

  @Watch("metaData")
  cmetaData(val, oldval) {
    if ((this.tableOption.table as any).lazy && val) {
      for (let item in this.$refs.multipleTable["store"].states.treeData) {
        this.$refs.multipleTable["store"].states.treeData[item].loaded = false;
        this.$refs.multipleTable["store"].states.treeData[
          item
        ].expanded = false;
      }

      // let key: any = this.table.rowKey;
      // debugger;
      // this.$refs.multipleTable["store"].states.treeData[key].loaded = false;
      // this.$refs.multipleTable["store"].states.treeData[key].expanded = false;
      //this.metaData.key = Math.random();
      // let key: any = this.table.rowKey;
      // this.$set(
      //   (this.$refs.multipleTable as any).store.states.lazyTreeNodeMap,
      //   0,
      //   val
      // );
    }
  ...
  /**
   * 点击切换列表展示形式table/card
   */
  private handleClick_listType() {
  ...
  private clickHandle(val: any, data: any, index: number) {
    switch (val) {
      case "search": {
        this.$refs.refQueryCard &&
          ((this.$refs.refQueryCard as any).currentCard = -1);
        //搜索
        this.init("search");
        break;
      }
