...
*/
@Component
export default class MThanosColorMixin extends Vue {
    @Action("setColorName") setColorName!: any;
    @Getter("getColorName") getColorName!: string;

    private colorVal: string = ORIGINAL_THEME; //颜色值
    private chalk: string = "";
    /**
   * 组件加载之前
   */
    private created() {
        this.colorVal = this.getColorName;
    }
     ...
     */
    @Watch("colorVal")
    private watchGetColorName(newVal: any, oldVal: any) {
        this.setColorName(newVal);
        ...
        for (let i = 0; i < link.length; i++) {
            ...
            if (style.href.includes("css")) {
                this.getCSSString(style.href, (innerText: any) => {
                    ...
                    let styleTag: any = document.getElementById(String(i));
                    if (!styleTag) {
                        styleTag = document.createElement("style");
                        styleTag.id = i;
                        styleTag.innerText = newStyle;
