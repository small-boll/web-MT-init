...
<template>
  ...
  >
    <eForm
      ...
      :loadState="false"
      :option="formOption"
      :dialogVis.sync="dialog"
      @doSubmit="doSubmit"
      @formSlot="formSlot"
    >
      <template
        v-for="(item, index) in formSlots"
        :slot="item.prop"
        slot-scope="scope"
      >
        <span :key="index">
    ...
    >
      <template
        v-for="(item, index) in detailSlots"
        :slot="item.prop"
        slot-scope="scope"
      >
        <span :key="index">
    ...
    <div slot="footer" class="dialog-footer">
      <el-button @click="cancel">{{ $t("cancelBtn") }}</el-button>
...
export default class MTDialogForm extends Vue implements BaseMethod {
  ...
  private loadDialog(newVal: boolean, oldVal: boolean) {
    ...
    this.formShow = Boolean(
      this.formOption.detailShow && this.chexckState1("eForm")
    );
  ...
  private doSubmit(type: string, form: any, index: number, done: any) {
    this.$emit("doSubmit", this.formOption.type, form, index, done);
  }
